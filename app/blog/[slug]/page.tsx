...
export async function generateMetadata({
  ...
  const p = demoPosts.find((x) => x.slug === slug);
  return { title: p ? `${p.title} — Saksham` : "Notebook — Saksham" };
}
...
export default async function PostPage({
  ...
  try {
    await connectDB();
    const found = await Post.findOne({ slug, published: true }).lean<any>();
    if (found) post = { ...found, _id: String(found._id) };
  } catch {}
  if (!post) notFound();
