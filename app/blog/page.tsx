...
export default async function BlogPage() {
  ...
  try {
    await connectDB();
    const found = await Post.find({ published: true })
      .sort({ publishedAt: -1 })
      .lean();
    if (found.length) posts = found.map((p) => ({ ...p, _id: String(p._id) }));
  } catch {}
  return (
          ...
          {posts.map((post, i) => (
            <Reveal key={post._id} delay={i * 0.05}>
              <Link
                data-cursor
                href={`/blog/${post.slug}`}
                className="group grid gap-6 rounded-[30px] border border-white/[.07] bg-[#111017] p-7 md:grid-cols-[140px_1fr_auto] md:items-start"
              >
                <div>
